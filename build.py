#!/usr/bin/env python3
"""Static site generator for the Coach Index Network.
...
def page(slug, title, desc, body, schema=None, canonical=None):
...
def build_index():
  ...
  <div class="body">
    <h2><a href="/coaches/{c['slug']}.html">{esc(c['name'])}</a></h2>
    ...
    parts.append('<h2>Ones to Watch</h2>')
    parts.append('<p>Named, not ranked. Practitioners whose work is strong and whose published '
  ...
  <ul class="caveats"><li>{w['note']}</li></ul>
    ...
    parts.append('<h2>Considered and not included</h2>')
    parts.append('<p>A ranking that quietly omits well-known names invites the assumption that it '
                 'did not know about them. These were assessed and left out for stated reasons.</p>')
    for x in DATA["not_included"]:
        parts.append('<div class="card"><h3>%s</h3><p>%s</p></div>' % (esc(x["name"]), x["text"]))

    parts.append('<h2>Verification still outstanding</h2>')
    parts.append('<p>Published openly rather than quietly resolved, because a methodology that '
    ...
    return page("index", S["title"] + " — " + S["edition"], S["tagline"],
                "\n".join(parts), schema, canonical=BASE + "/")
...
def build_coach(c):
    ...
    body = [f'<div class="meta">Ranked <strong>#{c["rank"]}</strong> in the {esc(S["edition"])} edition of '
            ...
            f"<h2>{esc(c['name'])}</h2>",
            f'<p class="where">{esc(c["location"])}</p>',
